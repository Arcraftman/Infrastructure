...
static int parse_proc_stat(pid_t pid,
                           char* state_out,
                           unsigned long* utime,
                           unsigned long* stime,
                           long* priority,
...
LNX_API lnx_proc_state_t lnx_proc_state(pid_t pid)
{
    ...
    switch (state) {
        case 'R':
            return LNX_PROC_STATE_RUNNING;
        case 'S':
            return LNX_PROC_STATE_SLEEPING;
        case 'D':
            return LNX_PROC_STATE_DISK_SLEEP;
        case 'Z':
            return LNX_PROC_STATE_ZOMBIE;
        case 'T':
            return LNX_PROC_STATE_STOPPED;
        case 't':
            return LNX_PROC_STATE_TRACING;
        case 'X':
            return LNX_PROC_STATE_DEAD;
        case 'I':
            return LNX_PROC_STATE_IDLE;
        default:
            return LNX_PROC_STATE_UNKNOWN;
    }
}
...
LNX_API lnx_proc_result_t lnx_proc_exec(const char* cmd)
{
    ...
    /* Read stdout */
    ssize_t n;
    while ((n = read(stdout_pipe[0], stdout_buf + stdout_n, sizeof(stdout_buf) - stdout_n - 1)) > 0)
        stdout_n += n;
    close(stdout_pipe[0]);

    while ((n = read(stderr_pipe[0], stderr_buf + stderr_n, sizeof(stderr_buf) - stderr_n - 1)) > 0)
        stderr_n += n;
    close(stderr_pipe[0]);
