...
STK_API STK_STATUS stk_log_write(stk_log_level_t level,
                                 const char* filename,
                                 int line,
                                 const char* func,
                                 const char* format,
...
#define STK_ASSERT(expr, format, ...)                             \
    do {                                                          \
        if (!(expr)) {                                            \
            stk_log_write(STK_LOG_LEVEL_FATAL,                    \
                          __FILE__,                               \
                          __LINE__,                               \
                          __func__,                               \
                          "Assertion failed: " #expr ". " format, \
                          ##__VA_ARGS__);                         \
            abort();                                              \
        }                                                         \
    } while (0)
...
#define STK_CHECK_PTR(ptr, format, ...) STK_RETURN_IF(!(ptr), STK_EINVAL, format, ##__VA_ARGS__)
...
#define STK_ERROR_RETURN(err, format, ...) return (err)

#define STK_ERROR_GOTO(label, format, ...) goto label
