    ...
    if (color) {
        switch (level) {
            case STK_LOG_LEVEL_TRACE:
                return STK_COLOR_TRACE "TRACE" STK_COLOR_RESET;
            case STK_LOG_LEVEL_DEBUG:
                return STK_COLOR_DEBUG "DEBUG" STK_COLOR_RESET;
            case STK_LOG_LEVEL_INFO:
                return STK_COLOR_INFO "INFO " STK_COLOR_RESET;
            case STK_LOG_LEVEL_WARN:
                return STK_COLOR_WARN "WARN " STK_COLOR_RESET;
            case STK_LOG_LEVEL_ERROR:
                return STK_COLOR_ERROR "ERROR" STK_COLOR_RESET;
            case STK_LOG_LEVEL_FATAL:
                return STK_COLOR_FATAL "FATAL" STK_COLOR_RESET;
            default:
                return "UNKNOWN";
        }
    } else {
...
static void log_write_impl(stk_log_level_t level,
                           const char* filename,
                           int line,
                           const char* func,
                           const char* format,
...
STK_STATUS stk_log_init(stk_log_config_t* config)
{
    ...
    stk_log_write(STK_LOG_LEVEL_INFO,
                  __FILE__,
                  __LINE__,
                  __func__,
                  "Logger initialized (level=%d)",
                  g_logger.config.log_level);
...
STK_STATUS stk_log_write(stk_log_level_t level,
                         const char* filename,
                         int line,
                         const char* func,
                         const char* format,
