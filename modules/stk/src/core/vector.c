...
void** stk_vector_at(stk_vector* v, size_t idx)
{
    if (!v || idx >= v->size) {
        if (v)
            STK_LOG_WARN("Vector at: index %zu out of range", idx);
        return NULL;
    }
    return &((void**)v->data)[idx];
}
...
void* stk_vector_front(stk_vector* v)
{
    if (!v || v->size == 0) {
        if (v)
            STK_LOG_WARN("Vector front: vector is empty");
        return NULL;
    }
...
void* stk_vector_back(stk_vector* v)
{
    if (!v || v->size == 0) {
        if (v)
            STK_LOG_WARN("Vector back: vector is empty");
        return NULL;
    }
