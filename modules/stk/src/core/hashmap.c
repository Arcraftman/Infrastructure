...
uint64_t stk_hashmap_ptr_hash(const void* key)
{
    /* Hash the pointer's numeric value. Folding mix to spread high bits. */
...
bool stk_hashmap_ptr_eq(const void* a, const void* b)
{
    return a == b;
}

uint64_t stk_hashmap_int_hash(const void* key)
{
    /* The integer is encoded directly in the pointer; hash its value. */
    uintptr_t v = (uintptr_t)key;
    uint64_t h = 0xcbf29ce484222325ULL;
    h ^= (uint64_t)v;
    h *= 0x100000001b3ULL;
...
bool stk_hashmap_int_eq(const void* a, const void* b)
{
    return a == b;
}
...
STK_STATUS stk_hashmap_init(stk_hashmap* m,
...
STK_STATUS stk_hashmap_create_str(stk_hashmap* m, size_t initial_capacity)
{
    return stk_hashmap_init(m, initial_capacity, stk_hashmap_str_hash, stk_hashmap_str_eq);
}

STK_STATUS stk_hashmap_create_ptr(stk_hashmap* m, size_t initial_capacity)
{
    return stk_hashmap_init(m, initial_capacity, stk_hashmap_ptr_hash, stk_hashmap_ptr_eq);
}

STK_STATUS stk_hashmap_create_int(stk_hashmap* m, size_t initial_capacity)
{
    return stk_hashmap_init(m, initial_capacity, stk_hashmap_int_hash, stk_hashmap_int_eq);
}
...
void* stk_hashmap_get(const stk_hashmap* m, const void* key)
{
    if (!m || !key) {
        if (m)
            STK_LOG_WARN("Hashmap get: NULL %s", !m ? "hashmap" : "key");
        return NULL;
    }
    size_t idx = hashmap_probe(m, key);
...
void* stk_hashmap_remove(stk_hashmap* m, const void* key)
{
    if (!m || !key) {
        if (m)
            STK_LOG_WARN("Hashmap remove: NULL %s", !m ? "hashmap" : "key");
        return NULL;
    }
...
STK_STATUS
stk_hashmap_foreach(const stk_hashmap* m, bool (*fn)(void* key, void* value, void* ud), void* ud)
{
...
size_t stk_hashmap_count(const stk_hashmap* m)
{
    return m ? m->count : 0;
}
size_t stk_hashmap_capacity(const stk_hashmap* m)
{
