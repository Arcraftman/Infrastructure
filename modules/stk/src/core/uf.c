...
size_t stk_uf_find(stk_uf* uf, size_t x)
{
    if (!uf || x >= uf->count) {
        if (uf)
            STK_LOG_WARN("UF find: invalid index %zu (max=%zu)", x, uf->count);
        return (size_t)-1;
    }
...
STK_STATUS stk_uf_union(stk_uf* uf, size_t x, size_t y)
{
    ...
    STK_RETURN_IF(x >= uf->count,
                  STK_ERANGE,
                  "UF union: x=%zu out of range (max=%zu)",
                  x,
                  uf->count - 1);
    STK_RETURN_IF(y >= uf->count,
                  STK_ERANGE,
                  "UF union: y=%zu out of range (max=%zu)",
                  y,
                  uf->count - 1);
...
bool stk_uf_connected(const stk_uf* uf, size_t x, size_t y)
{
    if (!uf || x >= uf->count || y >= uf->count) {
        if (uf)
            STK_LOG_WARN("UF connected: invalid index");
        return false;
    }
...
size_t stk_uf_set_size(const stk_uf* uf, size_t x)
{
    if (!uf || x >= uf->count) {
        if (uf)
            STK_LOG_WARN("UF set_size: invalid index %zu", x);
        return 0;
    }
