...
size_t stk_slist_find(const stk_slist* list,
                      const void* value,
                      bool (*equal)(const void* a, const void* b))
{
    if (!list || !equal) {
        if (list)
            STK_LOG_WARN("Slist find: NULL %s", !equal ? "equal function" : "list");
        return (size_t)-1;
    }
...
bool stk_slist_contains(const stk_slist* list,
                        const void* value,
                        bool (*equal)(const void* a, const void* b))
{
...
STK_STATUS
stk_slist_foreach(const stk_slist* list, bool (*fn)(void* data, void* user_data), void* user_data)
{
...
STK_STATUS
stk_slist_foreach_mut(stk_slist* list, bool (*fn)(void** data, void* user_data), void* user_data)
{
...
size_t stk_slist_remove_all(stk_slist* list,
                            const void* value,
                            bool (*equal)(const void* a, const void* b))
{
    if (!list || !equal) {
        if (list)
            STK_LOG_WARN("Slist remove_all: NULL %s", !equal ? "equal function" : "list");
        return 0;
    }
...
stk_snode* stk_slist_node_at(const stk_slist* list, size_t index)
{
    if (!list || index >= list->size) {
        if (list)
            STK_LOG_WARN("Slist node_at: index %zu out of range", index);
        return NULL;
    }
