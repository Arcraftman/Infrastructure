...
static void sha1_transform(uint32_t state[5], const unsigned char block[SHA1_BLOCK_SIZE])
{
    uint32_t w[80];
    for (int i = 0; i < 16; i++)
    ...
    uint32_t a = state[0], b = state[1], c = state[2], d = state[3], e = state[4], f, tmp;

#define SHA1_ROUND(i, K)                                 \
    do {                                                 \
        if (i < 20)                                      \
            f = (b & c) | (~b & d);                      \
        else if (i < 40)                                 \
    ...
    for (int i = 0; i < 80; i++) {
        if (i < 20)
            SHA1_ROUND(i, 0x5A827999);
        else if (i < 40)
            SHA1_ROUND(i, 0x6ED9EBA1);
        else if (i < 60)
            SHA1_ROUND(i, 0x8F1BBCDC);
        else
            SHA1_ROUND(i, 0xCA62C1D6);
    }
...
WEB_API int web_ws_is_upgrade(const web_request_t* req)
{
    if (!req)
        return 0;
    ...
    if (!conn)
        return 0;
    if (strcasestr(conn, "Upgrade") == NULL)
        return 0;

    return 1;
}

WEB_API int web_ws_accept(const web_request_t* req, web_response_t* resp)
{
    if (!req || !resp)
        return -1;
...
WEB_API int web_ws_read_frame(int fd, web_ws_frame_t* frame)
{
    if (!frame)
        return -1;
    ...
    return (int)(2 + (plen > 125 ? (plen > 65535 ? 8 : 2) : 0) + (frame->masked ? 4 : 0) + plen);
}

WEB_API int web_ws_write_frame(int fd, const void* data, size_t len, unsigned char opcode)
{
    ...
    if (write(fd, header, hdr_len) < (ssize_t)hdr_len)
        return -1;
...
WEB_API void web_ws_frame_destroy(web_ws_frame_t* frame)
{
    if (frame)
        free(frame->payload);
}
