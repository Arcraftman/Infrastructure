...
#define WEB_MAX_REQUEST_SIZE (16U * 1024U * 1024U)
...
static web_connection_t* conn_new(web_server_t* srv, int fd)
{
    ...
    if (fd >= srv->conn_cap) {
        int new_cap = fd + 64;
        web_connection_t** newc =
            (web_connection_t**)realloc(srv->conns, (size_t)new_cap * sizeof(*newc));
        if (!newc) {
            free(c->buf);
            free(c);
            return NULL;
        }
        memset(newc + srv->conn_cap, 0, (size_t)(new_cap - srv->conn_cap) * sizeof(*newc));
        srv->conns = newc;
...
static void conn_close(web_connection_t* c)
{
    if (!c)
        return;
    if (c->fd >= 0) {
...
static web_route_t* find_route(web_server_t* srv, const char* method, const char* path)
{
    for (web_route_t* r = srv->routes; r; r = r->next) {
        /* Method match */
        int method_match = (strcmp(r->method, "*") == 0 || strcmp(r->method, method) == 0);
        if (!method_match)
            continue;
...
static web_response_t* serve_static_file(web_server_t* srv, const char* path)
{
    struct web_static* st;
    const char* rel = NULL;
    for (st = srv->statics; st; st = st->next) {
        if (strncmp(st->mount, path, st->mount_len) == 0 &&
            (path[st->mount_len] == '\0' || path[st->mount_len] == '/')) {
            rel = path + st->mount_len;
            break;
        }
    }
    if (!st || (rel && strstr(rel, "..")))
        return NULL;
    ...
    if (!S_ISREG(stbuf.st_mode) || size > WEB_MAX_REQUEST_SIZE) {
        fclose(fp);
        return web_response_error(WEB_STATUS_PAYLOAD_TOO_LARGE);
    }
    content = (unsigned char*)malloc(size ? size : 1);
    if (!content) {
        fclose(fp);
...
static void handle_connection(web_connection_t* c)
{
...
static int accept_connection(web_server_t* srv)
{
...
static int read_request(web_connection_t* c)
{
    ssize_t n;
    do {
        if (c->buf_len >= WEB_MAX_REQUEST_SIZE)
            return -1;
        if (c->buf_len >= c->buf_cap) {
            size_t new_cap = c->buf_cap < WEB_MAX_REQUEST_SIZE / 2
                                 ? c->buf_cap * 2
                                 : WEB_MAX_REQUEST_SIZE;
            char* nb = (char*)realloc(c->buf, new_cap);
            if (!nb)
                return -1;
            c->buf = nb;
            c->buf_cap = new_cap;
        }
        n = read(c->fd, c->buf + c->buf_len, c->buf_cap - c->buf_len - 1);
        if (n > 0)
            c->buf_len += (size_t)n;
    } while (n > 0);
    ...
    if (c->buf_len >= 4 && memmem(c->buf, c->buf_len, "\r\n\r\n", 4)) {
        web_request_t* request = NULL;
        if (web_request_parse(c->buf, c->buf_len, &request) == 0) {
            web_request_destroy(request);
            c->buf[c->buf_len] = '\0';
            handle_connection(c);
        } else {
            const char* header_end = memmem(c->buf, c->buf_len, "\r\n\r\n", 4);
            const char* content_length = header_end ? strcasestr(c->buf, "Content-Length:") : NULL;
            unsigned long long expected = 0;
            if (content_length && (!header_end || content_length < header_end))
                expected = strtoull(content_length + 15, NULL, 10);
            if (!content_length || (size_t)(header_end + 4 - c->buf) + (size_t)expected <= c->buf_len) {
                c->buf[c->buf_len] = '\0';
                handle_connection(c);
            }
        }
...
static int write_response(web_connection_t* c)
{
...
static void event_loop_epoll(web_server_t* srv)
{
    while (srv->running) {
        ...
        int nfds = epoll_wait(srv->epoll_fd, srv->events, WEB_MAX_EVENTS, 1000);
        if (nfds < 0) {
            if (errno == EINTR)
                continue;
            perror("epoll_wait");
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (fd == srv->listen_fd) {
                ...
                if ((srv->events[i].events & EPOLLIN) && c->state == CONN_READING) {
                    if (read_request(c) < 0) {
                        conn_close(c);
                        continue;
                    }
...
static void event_loop_select(web_server_t* srv)
{
    while (srv->running) {
        /* Check for timed-out connections */
        if (srv->timeout > 0) {
        ...
        int ret = select(max_fd + 1, &srv->read_fds, &srv->write_fds, NULL, &tv);
        if (ret < 0) {
            if (errno == EINTR)
                continue;
            perror("select");
        ...
        for (int i = 0; i < srv->conn_cap; i++) {
            ...
            if (c->state == CONN_READING && FD_ISSET(c->fd, &srv->read_fds)) {
                if (read_request(c) < 0) {
                    conn_close(c);
                    continue;
                }
            }
            if (c->state == CONN_WRITING && FD_ISSET(c->fd, &srv->write_fds)) {
                if (write_response(c) < 0) {
                    conn_close(c);
                    continue;
                }
...
WEB_API web_server_t* web_server_create(const char* addr, int port)
{
...
WEB_API int web_server_set_backlog(web_server_t* srv, int backlog)
{
    if (!srv)
        return -1;
    srv->backlog = backlog;
...
WEB_API int web_server_set_threads(web_server_t* srv, int threads)
{
    if (!srv)
        return -1;
    srv->threads = threads;
...
WEB_API int web_server_set_timeout(web_server_t* srv, int seconds)
{
    if (!srv || seconds < 0)
        return -1;
    srv->timeout = seconds;
...
WEB_API int web_server_set_keepalive(web_server_t* srv, int enabled)
{
    if (!srv)
        return -1;
...
WEB_API int web_server_add_route(web_server_t* srv,
                                 const char* method,
                                 const char* path,
                                 web_handler_fn handler,
                                 void* userdata)
{
    return web_server_add_route_ex(srv, method, path, handler, userdata, NULL);
}

WEB_API int web_server_add_route_ex(web_server_t* srv,
                                    const char* method,
                                    const char* path,
                                    web_handler_fn handler,
                                    void* userdata,
                                    web_handler_dtor dtor)
{
...
WEB_API int web_server_add_static(web_server_t* srv, const char* mount, const char* dir)
{
    if (!srv || !mount || !dir)
        return -1;

    struct web_static* st = (struct web_static*)calloc(1, sizeof(*st));
    if (!st)
        return -1;
...
WEB_API int web_server_start(web_server_t* srv)
{
    if (!srv)
        return -1;
    ...
    /* Create a socket matching the configured address family. */
    int use_ipv6 = strchr(srv->addr, ':') != NULL;
    ...
    if (use_ipv6) {
        if (inet_pton(AF_INET6, srv->addr, &addr6.sin6_addr) != 1) {
            close(fd);
            return -1;
        }
    }
    ...
    if (use_ipv6) {
        ...
        if (inet_pton(AF_INET, srv->addr, &addr4.sin_addr) != 1) {
            close(fd);
            return -1;
        }
...
WEB_API void web_server_stop(web_server_t* srv)
{
    if (srv)
        srv->running = 0;
}

WEB_API void web_server_destroy(web_server_t* srv)
{
    if (!srv)
        return;
