...
WEB_API int web_uri_encode(const char* src, size_t len, char** out)
{
    if (!out)
        return -1;
    if (!src) {
...
WEB_API size_t web_uri_decode(char* str)
{
    if (!str)
        return 0;
    size_t r = 0, w = 0;
...
WEB_API int web_uri_parse(const char* str, size_t len, web_uri_t** out)
{
    if (!str || !out)
        return -1;
    if (len == 0)
...
WEB_API char* web_uri_build(const web_uri_t* uri)
{
    if (!uri || !uri->path)
        return NULL;
...
WEB_API void web_uri_destroy(web_uri_t* uri)
{
    if (!uri)
        return;
    free(uri->scheme);
