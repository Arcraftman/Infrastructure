...
WEB_API int web_auth_basic_parse(const char* header, char** username, char** password)
{
...
WEB_API char* web_auth_basic_encode(const char* username, const char* password)
{
    if (!username || !password)
        return NULL;
...
WEB_API web_response_t* web_auth_basic_challenge(const char* realm)
{
    web_response_t* resp = web_response_new(WEB_STATUS_UNAUTHORIZED);
    if (!resp)
        return NULL;
    ...
    web_response_set_body_copy(resp, "401 Unauthorized\n", strlen("401 Unauthorized\n"));
    return resp;
}
...
WEB_API char* web_auth_bearer_parse(const char* header)
{
    if (!header)
        return NULL;
...
WEB_API web_response_t* web_auth_bearer_challenge(const char* scope)
{
    web_response_t* resp = web_response_new(WEB_STATUS_UNAUTHORIZED);
    if (!resp)
        return NULL;
