...
WEB_API web_compress_algo_t web_compress_negotiate(const char* accept_encoding)
{
    if (!accept_encoding)
...
WEB_API const char* web_compress_name(web_compress_algo_t algo)
{
    switch (algo) {
        case WEB_COMPRESS_GZIP:
            return "gzip";
        case WEB_COMPRESS_DEFLATE:
            return "deflate";
        default:
            return NULL;
    }
...
WEB_API int
web_compress(web_compress_algo_t algo, const void* in, size_t in_len, void** out, size_t* out_len)
{
    if (!in || !out || !out_len)
        return -1;
    *out = NULL;
    *out_len = 0;
    ...
    int ret =
        deflateInit2(&strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED, window_bits, 8, Z_DEFAULT_STRATEGY);
    if (ret != Z_OK)
        return -1;
...
WEB_API int web_response_compress(web_compress_algo_t algo, web_response_t* resp)
{
    if (!resp || algo == WEB_COMPRESS_NONE)
        return -1;

    if (!resp->body || resp->body_len == 0) {
        /* Nothing to compress — just set the header */
        return web_response_set_header(resp, "Content-Encoding", web_compress_name(algo));
    }

    void* compressed = NULL;
    size_t compressed_len = 0;
    if (web_compress(algo, resp->body, resp->body_len, &compressed, &compressed_len) != 0)
        return -1;
    ...
    if (web_response_set_header(resp, "Content-Encoding", web_compress_name(algo)) != 0) {
