...
WEB_API web_cookie_t* web_cookie_parse(const char* header)
{
    if (!header || !*header)
        return NULL;
...
WEB_API web_cookie_t* web_cookie_parse_set(const char* header)
{
    if (!header || !*header)
        return NULL;
...
WEB_API char* web_cookie_serialize(const web_cookie_t* cookie)
{
...
WEB_API char* web_cookie_serialize_set(const web_cookie_t* cookie)
{
    if (!cookie || !cookie->name)
        return NULL;
    ...
    char attr_buf[128];
    if (cookie->domain)
        len += 8 + strlen(cookie->domain);
    if (cookie->path)
        len += 6 + strlen(cookie->path);
    if (cookie->secure)
        len += 8;
    if (cookie->httponly)
        len += 10;
    ...
    size_t pos = sprintf(out, "%s=%s", cookie->name, cookie->value ? cookie->value : "");
    if (cookie->path)
...
WEB_API void web_cookie_free(web_cookie_t* cookie)
{
    while (cookie) {
...
WEB_API web_cookie_jar_t* web_cookie_jar_create(void)
{
    web_cookie_jar_t* jar = (web_cookie_jar_t*)calloc(1, sizeof(*jar));
    return jar;
}

WEB_API int web_cookie_jar_store(web_cookie_jar_t* jar,
                                 const char* set_cookie_header,
                                 const char* domain,
                                 const char* path)
{
    if (!jar || !set_cookie_header)
        return -1;
    ...
    /* apply defaults */
    if (!ck->domain)
        ck->domain = strdup(domain ? domain : "");
    if (!ck->path)
        ck->path = strdup(path ? path : "/");
    ...
    while (*pp) {
        jar_entry_t* e = *pp;
        if (strcmp(e->cookie->name, ck->name) == 0 && strcmp(e->domain, ck->domain) == 0 &&
...
static int domain_match(const char* cookie_domain, const char* request_domain)
{
...
static int path_match(const char* cookie_path, const char* request_path)
{
    ...
    if (strncmp(cookie_path, request_path, plen) != 0)
        return 0;
    /* exact match or request path continues with '/' */
    return request_path[plen] == '\0' || request_path[plen] == '/';
}

WEB_API char* web_cookie_jar_get(web_cookie_jar_t* jar, const char* domain, const char* path)
{
    if (!jar || !domain || !path)
        return NULL;
    ...
    for (jar_entry_t* e = jar->entries; e; e = e->next) {
        ...
        if (!path_match(e->path, path))
            continue;
        total += strlen(e->cookie->name) + 1 + (e->cookie->value ? strlen(e->cookie->value) : 0) +
                 2; /* "; " */
        count++;
    }
...
WEB_API void web_cookie_jar_cleanup(web_cookie_jar_t* jar)
{
    if (!jar)
        return;
    time_t now = time(NULL);
...
WEB_API void web_cookie_jar_destroy(web_cookie_jar_t* jar)
{
    if (!jar)
        return;
