...
static void form_decode_inplace(char* s)
{
    if (!s)
        return;
    size_t r = 0, w = 0;
...
WEB_API web_form_t* web_form_parse_urlencoded(const void* data, size_t len)
{
    if (!data || len == 0)
        return NULL;
...
WEB_API web_form_t* web_form_create(void)
{
    return (web_form_t*)calloc(1, sizeof(web_form_t));
}

WEB_API int web_form_add_field(web_form_t* form,
                               const char* name,
                               const void* value,
                               size_t value_len,
                               const char* filename,
...
WEB_API const web_form_field_t* web_form_get(const web_form_t* form, const char* name)
{
    if (!form || !name)
        return NULL;
...
WEB_API const char* web_form_get_value(const web_form_t* form, const char* name)
{
    const web_form_field_t* f = web_form_get(form, name);
    return f ? f->value : NULL;
}

WEB_API void web_form_destroy(web_form_t* form)
{
    if (!form)
        return;
