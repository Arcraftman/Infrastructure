...
web_session_get(web_session_store_t* store, const char* session_id, const char** new_session_id);
 ...
 */
WEB_API const char* web_session_get_value(const web_session_t* sess, const char* key);

/**
 * Destroy a session (removes it from the store).
 */
WEB_API void web_session_destroy(web_session_store_t* store, web_session_t* sess);
